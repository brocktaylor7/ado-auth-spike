...
async function authenticate(page: puppeteer.Page, attemptNumber: number = 1): Promise<void> {
    await page.goto('https://portal.microsofticm.com/imp/v3/overview/main');

    await Promise.all([page.waitForNavigation(), page.click('button[name="MSIT-ADFS-Federation"')]);

    await page.waitForSelector('input[type="email"');
    await page.type('input[type="email"]', process.env.SERVICE_ACCT_USER!);
